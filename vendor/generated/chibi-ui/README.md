...
- `source/seating-rework/guest-*-sheet-{keyed,transparent}.png` — four-direction
  ...
  same master. Runtime diners keep those five adult identities (hair, outfit,
  face) packed at one shared scale and feet baseline so table fit and status
  dots stay aligned.
...
- `guest-frames/` — runtime 128×160 frames derived from each adult identity
  sheet at one shared scale and one feet baseline. Seated poses are authored,
  not synthesized.
