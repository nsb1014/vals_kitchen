...
- `source/seating-rework/guest-*-sheet-{keyed,transparent}.png` — four-direction
  ...
  same master. Runtime diner looks are palette swaps of the adult `guest-e`
  template so every guest shares one silhouette, table fit, and status-dot
  height. The other identity sheets are retained as generation history.
...
- `guest-frames/` — runtime 128×160 frames derived from the adult master sheet
  at one fixed scale and one feet baseline; variants a–d are palette swaps of
  those frames (identical alpha). Seated poses are authored, not synthesized.
