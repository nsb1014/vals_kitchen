...
async function expectPlateOnlyMobileFooter(page: Page): Promise<void> {
  await expect(page.getByTestId("compose-flavor-toggle")).toBeHidden();
  await expect(page.locator(".compose-flavor-strip")).toBeHidden();
  await expect(page.locator(".compose-footer-copy")).toBeHidden();
...
test.describe("cook sheet responsive chrome", () => {
  for (const width of [320, 360, 390]) {
    test(`holds fixed regions with the full pantry at ${width}px`, async ({
      ...
      await expect(page.getByTestId("ingredient-inspect").first()).toHaveText(
        "i",
      );
      ...
      const firstCard = await page
        .locator(".compose-ingredient-card")
        .first()
        .boundingBox();
      const firstIngredient = await page
        .getByTestId("ingredient-chip")
        .first()
        .boundingBox();
      const firstInspect = await page
        .getByTestId("ingredient-inspect")
        .first()
        .boundingBox();
  ...
  test("keeps a plate-only mobile footer without flavor details", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 390, height: 720 });
    await openCookFixture(page);
    await expect(page.locator(".compose-selection")).toBeHidden();
    await expect(page.getByTestId("status-mount")).toBeHidden();
    await expectPlateOnlyMobileFooter(page);
  });
  ...
  test("inspects by pointer and keyboard without changing the selected draft", async ({
    ...
    // Inspect uses a large layout box with pointer-events on the badge only so
    // chip body taps still select ingredients.
    await firstInspect.locator("span").click();
    await expect(
  ...
  test("keeps the desktop restaurant fixed behind the readable modifier sheet", async ({
    ...
    await expect(
      page.getByRole("button", { name: "Cash details" }),
    ).toBeVisible();
    await expect(
...
test.describe("service sheet tiers", () => {
  ...
  test("gives ceremony precedence over a pending review and restores review focus", async ({
    ...
    await waitForServiceStarted(page);
    await serveCurrentCustomer(page);
    await page.evaluate(() => window.__E2E__!.showCeremonyOverPendingReview());
