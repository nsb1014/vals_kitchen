...
| Date | Change |
|------|--------|
| 2026-08-12 | Polish loop 4: disabled all actor bounce (walk squash / idle breathe / chew pulse); cooking sheet hides status HUD and keeps a Plate-only footer with a compact selected strip and smaller pantry chips; reverted uniform sit raise so taller diners stop floating; fixed guest-b right walk frame 2 facing the wrong way (child flip-flop) with build-time side-facing correction; fast suite pending |
| 2026-08-12 | Polish loop 3: interact hints mark service cells (not stools) and show at most one far tile; walk facing follows visual travel with hysteresis (no flip-flop); doorway crop masks stay non-renderable and skip head cues mid-threshold; seated guests raised 8px so faces clear tabletops; speech bubbles stack above the closed Tickets toggle; Plate CTA shortened to `Plate · Table N`; mobile compose gives the pantry the sheet height by capping Flavor Targets and trimming chrome — gameplay rules unchanged; fast suite 714 |
| 2026-08-11 | Polish loop 2: removed the vertical idle bob entirely (player + waiting guests keep breathe scale only — rhythmic lift read as scene bounce); stools now tuck toward the table by the same hip shift as seated guests so sit poses center on cushions (guest anchors/depths unchanged); guestServicePositions prefers the four one-cell tableside neighbors with two-cell vertical cells as fallbacks, so order/deliver stands beside the table — strand/repair fixtures redesigned and verified against the real flood-fill + repair search; fast suite 712; e2e 136; gzip 251,003 |
| 2026-08-11 | Floor/UI polish loop: seated diners keep hips on visible stools (no bob, corrected hip offsets); walking uses path-global Hermite easing with continuous speed across tile boundaries and distance-phased walk frames; serve delivery is a gentle place-down (steam + sparkles, soft canvas flash) instead of a star burst; guest taps resolve via padded sprite bounds + nearest-guest fallback; top banner reserved for celebrations/actionable toasts while tutorial/pacing copy moved to a quiet hud-hint line with toast coalescing; Skip tutorial now clears guidance through a store action that HUD sync cannot resurrect; viewport scroll locked so menus no longer page-scroll; status HUD stacks above the canvas; compose sheet auto-selects the next ticket, shows Selected n/6 + Clear dish, destination-labeled Plate CTA with disabled reasons; fast suite 712; initial JS gzip 251,042 / 280k |
| 2026-08-03 | Gameplay continuity: save v7 now persists mandatory customer reviews, completed-day summaries, reset ceremonies, day-start rating context, and recent review history through reload and Save Codes; retryable acknowledgement writes fence later autosaves so cleared checkpoints remain cleared, with v1–v6 migration and backup recovery preserved |
