...
describe("character-frames", () => {
  ...
  it("hashes guest ids stably across the full variant set", () => {
    const counts = Object.fromEntries(
      GUEST_VARIANTS.map((v) => [v, 0]),
    ) as Record<(typeof GUEST_VARIANTS)[number], number>;
  ...
  it("builds walk and sit texture keys per variant", () => {
    for (const variant of GUEST_VARIANTS) {
      expect(guestWalkFrameKey(variant, "right", 1)).toBe(
        `guest_${variant}_right_1`,
      );
      expect(guestSitFrameKey(variant, "down")).toBe(
        `guest_${variant}_sit_down`,
      );
    }
  });
});
