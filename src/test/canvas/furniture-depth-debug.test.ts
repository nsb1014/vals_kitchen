...
describe('furniture seating depth debug', () => {
  it('reports table and stool roots in deterministic identity order', () => {
    ...
    expect(layer.getSeatingDepthDebug()).toEqual({
      ...
      // NS 4px). Sort uses guest feet (including sit raise) so stools stay
      // behind diners after the face-clearing sit offset.
      chairs: [
        { tablePlacementId: 'table_a', slotIndex: 0, zIndex: 57, x: 64, y: 34 },
        { tablePlacementId: 'table_a', slotIndex: 1, zIndex: 85, x: 70, y: 62 },
        { tablePlacementId: 'table_b', slotIndex: 1, zIndex: 113, x: 128, y: 90 },
      ],
    });
  });
});
