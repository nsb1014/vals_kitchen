...
/** Runtime display heights are independent from high-resolution source frames. */
const PLAYER_DISPLAY_HEIGHT = 62;
...
export class ActorLayer {
  ...
  private drawDestination(dest: GridPoint | null): void {
    ...
    this.markerLayer
      .circle(cx, cy, 5)
      .stroke({ width: 2, color: DEST_MARKER_COLOR, alpha: 0.85 });
    this.markerLayer
      .circle(cx, cy, 2)
      .fill({ color: DEST_MARKER_COLOR, alpha: 0.9 });
  }
    ...
    if (
      ...
      if (texture) {
        ...
        this.playerSprite.scale.set(
          scaleForTexture(texture, PLAYER_DISPLAY_HEIGHT),
        );
      ...
      if (
        ...
        if (texture) {
          ...
          entry.sprite.scale.set(
            scaleForTexture(texture, GUEST_DISPLAY_HEIGHT),
          );
